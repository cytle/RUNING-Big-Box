function fireworksConsole(){
	...
	function fireworkRun(){
		...
		while(temp=tempMap.shift()){
			if(temp.run()){
				console.log(substancesArr.length+"mmm");
...
function fireworksObj(x,y,w,h,fireHeight){
	...
	o.fireOpen=function(substancesArr){
		...
		for (var i = 0; i <= 360; i+=20) {
			for (var timeout = 0; timeout <= 10; timeout+=2) {
				substancesArr.push(new substancesObj(o.x,o.y,-g*(13-timeout),i,timeout*100));
				substancesArr.push(new substancesObj(o.x,o.y,-g*(12-timeout),i+10,(timeout+1)*100));
			};
		};
	}
}
